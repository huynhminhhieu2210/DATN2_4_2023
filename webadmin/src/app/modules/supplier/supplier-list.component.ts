...
export class SupplierListComponent extends ComponentBase implements OnInit{
  ...
  onEdit(item: any){
    this.navigatePassParam('/supplier-edit', { supplier: item }, { filterInput: JSON.stringify(undefined) });
  }
  onView(item: any){
    this.navigatePassParam('/supplier-view', { supplier: item }, { filterInput: JSON.stringify(undefined) });
  }
  onDel(){
